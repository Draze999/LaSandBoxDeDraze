...
export class RorschachEngine {
  ...
  private finishDrawing(code:string){const s=this.states.get(code);if(!s||s.phase!=="drawing")return;s.phase="guessing";s.currentPlayerIndex=0;s.guesses=[];this.onState(code);}
  ...
  validate(code:string,playerId:string){const s=this.states.get(code);if(!s||s.phase!=="drawing")return{ok:false as const,error:"NOT_DRAWING"};s.validated[playerId]=true;if(s.playerOrder.every(id=>s.validated[id]))this.finishDrawing(code);else this.onState(code);return{ok:true as const};}
