  ...
  if (game.phase === "guessing") {
    ...
    return <main className="game4-page"><section className="game4-shell">
      ...
      <div className="game4-review-layout"><div><TierlistView snapshot={game} board={board} editable={false} /></div><aside className="game4-side">
        {own ? <div className="game4-info"><span>Ton thème</span><strong>{game.theme}</strong><p>Les autres joueurs doivent le retrouver.</p></div> : <div className="game4-guess-form"><label>Ta proposition</label><select value={guess} onChange={e=>setGuess(e.target.value)} disabled={already}>
          <option value="">Choisis un thème…</option>
          {(game.category === "anime"
            ? ["Le meilleur scénario","Le meilleur Worldbuilding","La meilleure ambiance","Les meilleures musiques","Le meilleur opening","Le meilleur ending","Le meilleur style visuel","Le meilleur humour","Le meilleur développement de personnage","La meilleure romance","La meilleure amitié","Le meilleur combat","Les plus sous-côtés","À revoir encore une fois sans hésiter","Recommandations à quelqu'un qui débute les animés","Pour une soirée tranquille","À regarder avec des amis","Le meilleur potentiel de crossover","Mérite une suite","Mérite un spin-off","Le plus de potentiel inexploité","Le potentiel de mème","Les animés avec le meilleur premier épisode","Les animés avec la meilleure fin","Les animés qui vieillissent le mieux","Les animés que tu recommanderais à tout le monde","Les animés les plus marquants","Les animés qui méritent plus de reconnaissance"]
            : ["Le meilleur protagoniste","Le meilleur antagoniste","Le meilleur deutéragoniste","Le meilleur professeur","Le meilleur chef d'équipe","Le meilleur compagnon de voyage","Le meilleur allié","Le meilleur ennemi","Le meilleur charisme","La meilleure évolution","Le meilleur style","Le meilleur humour","Le personnage le plus dangereux","Le personnage le plus intelligent","Le personnage le plus attachant","Le personnage que tu voudrais comme ami","Le personnage que tu voudrais comme voisin","Le personnage avec le meilleur design","Le personnage avec le meilleur développement","Le personnage le plus sous-côté","Le personnage le plus mémorable","Le meilleur potentiel de mème","Qui survivrait le mieux dans le monde réel","Qui serait le plus dangereux dans la vraie vie","Celui qui te manque le plus","Tes préférés"]
          ).map(theme => <option key={theme} value={theme}>{theme}</option>)}
        </select><button className="primary purple" disabled={already || !guess} onClick={submitGuess}>{already ? "Proposition envoyée" : "Proposer le thème →"}</button></div>}
